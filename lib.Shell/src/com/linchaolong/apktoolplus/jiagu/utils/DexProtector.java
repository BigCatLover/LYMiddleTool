...
//			mClassLoaderField.setAccessible(true);
     ...
     * 加载加密后的dex文件
     *
     * @param dexList 加密的dex文件列表
     * @return ClassLoader
     */
    public ClassLoader loadEncryptDex(String[] dexList) {
        try {
            //通过反射修改ActivityThread中LoadedApk的ClassLoader字段
            ClassLoader appClassLoader =  getAppClassLoader();
            String pahts="";
            for (int i = 0; i < dexList.length; i++) {
                pahts+=createEncryptDexLoader(context.getAssets().open("jiagu_data.bin/" + dexList[i]), dexList[i])+":"; // 优先系统的
            }
            // 经过优化的dex输出目录
     ...
     * @return ClassLoader
     */
    public String createEncryptDexLoader(InputStream in, String outFileName) {
        // 解密dex文件的File对象
